...
try {
    ...
    $gamesSummary = getSelectedGamesSummary($pdo, $_SESSION['kiosk_game_ids'] ?? '');
    if (!empty($gamesSummary['games'])) {
        $smsMessage .= "Games: " . implode(', ', array_column($gamesSummary['games'], 'name')) . "\n";
    }
